...
var jsonDsnSnowflakeDeltaDate = `{
  "schemaVersion": 3,
  "description": "cp delta from DSN to Snowflake using DATE field",
  "connections": {
    "source": {
      "type": "${srcType}",
      ...
      "steps": {
        "getMaxDateInTarget": {
          ...
          "data": {
            ...
            "sqlText": "select nvl(max(${SQLBatchDriverField}), ${deltaDateTemplateBatchStartDateTime}) \"#maxDateInTarget\" from ${snowflakeTable}"
          }
        },
