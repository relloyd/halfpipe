...
func SetupCpOraSnowflakeDelta(genericCfg interface{}, actionCfg interface{}) error {
	...
	tgt := actionCfg.(*OraSnowflakeDeltaConfig)
	var err error
...
func RunSnowflakeDelta(cfg interface{}) error {
	cfgDelta := cfg.(*OraSnowflakeDeltaConfig)
	// Setup logging.
	...
	tableCols, err := td.GetTableColumns(log, td.GetColumnsFunc(cfgDelta.SrcConnDetails), &cfgDelta.OraSchemaTable)
	if err != nil {
		return err
	}
	pkTokens, otherTokens, err := getKeysAndOtherColumns(cfgDelta.SQLPrimaryKeyFieldsCsv, tableCols)
	if err != nil {
		return errors.Wrap(err, fmt.Sprintf("some of the supplied fields are not present in table/view %q", cfgDelta.OraSchemaTable.SchemaTable))
	}
	colList := helper.EscapeQuotesInString(helper.StringsToCsv(tableCols))
	// Check data type of driver field.
	deltaDriverDataType, err := td.ColumnIsNumberOrDate(log, td.GetColumnsFunc(cfgDelta.SrcConnDetails), td.MustGetMapper(cfgDelta.SrcConnDetails), &cfgDelta.OraSchemaTable, cfgDelta.SQLBatchDriverField)
	if err != nil {
		return err
	}
	...
	// Source
	m["${sourceEnv}"] = cfgDelta.SourceConnection
	m["${sourceDsn}"] = connSrc.Dsn
	m["${sourceTable}"] = cfgDelta.OraSchemaTable.SchemaTable
	...
	if deltaDriverDataType == 0 { // if the field is of type NUMBER...
		...
		jsonPipe = &jsonOracleSnowflakeDeltaNumber
		...
		jsonPipe = &jsonOracleSnowflakeDeltaDate
	} else {
