...
func SetupCpDsnSnowflakeSnap(genericCfg interface{}, actionCfg interface{}) error {
	...
	tgt := actionCfg.(*DsnSnowflakeSnapConfig)
	var err error
...
func RunDsnSnowflakeSnapshot(cfg interface{}) error {
	cfgSnap := cfg.(*DsnSnowflakeSnapConfig)
	// Setup logging.
	...
	tableCols, err := tabledefinition.GetTableColumns(log, tabledefinition.GetColumnsFunc(cfgSnap.SrcConnDetails), &cfgSnap.DsnSchemaTable)
	if err != nil {
		return err
	}
	...
	m["${sourceTable}"] = cfgSnap.DsnSchemaTable.SchemaTable
	...
	mustReplaceInStringUsingMapKeyVals(&jsonDsnSnowflakeLoaderSnapshot, m)
	log.Debug("replaced reference JSON for snapshot load ", jsonDsnSnowflakeLoaderSnapshot)
	...
	if cfgSnap.ExportConfigType == "" { // if we should execute the transform...
		...
		_, err := transform.LaunchTransformJson(log, ti, jsonDsnSnowflakeLoaderSnapshot, true, cfgSnap.StatsDumpFrequencySeconds)
		if err != nil {
		...
		return outputPipeDefinition(log, jsonDsnSnowflakeLoaderSnapshot, cfgSnap.ExportConfigType, cfgSnap.ExportIncludeConnections)
	}
	return nil
}
